#include<bits/stdc++.h>
using namespace std;
string hextobin(string p)//Hex ke binary
{
    string ap="";
    int l=p.length();
    for(int i=0;i<l;i++)
...
int main()
{
        ...
        //Syarat inputan harus 16 bit hex
        int key1[64];
        pr:;
                cin>>ke;
         if(ke.length()!=16)
    ...
    for(i=0;i<16;i++)
    {
        if(i==0||i==1||i==8||i==15)//Shift 1 kali
            nshift=1;
        else
            nshift=2;//Shift 2 kali selain diatas
        while(nshift--)//Shifting
        {
            temp1=keyl[0];
            temp2=keyr[0];
            for(j=0;j<27;j++)
    ...
    //ENKRIPSI DES
         while(round--)
         {
            ...
            //XOR
            for(i=0;i<48;i++)
                exor+=char(((int(ep[i])-48)^key[16-round-1][i])+48);
            //SBOX KOMPRESI 48->32
            for(i=0;i<48;i+=6)
            {
                    row=(int(exor[i+5])-48)+(int(exor[i])-48)*2;//matrix 2 dimensi
                    col= (int(exor[i+1])-48)*8+(int(exor[i+2])-48)*4+(int(exor[i+3])-48)*2+(int(exor[i+4])-48);
                    temp=s[i/6][row][col];
                    soutt="";
                    while(temp>0)
                    {
            ...
            l=rtem;
            cout<<"Enkripsi Round"<<16-round<<endl;
            string cip="";
            for(i=0;i<32;i+=4)
            {
                int te;
    ...
    while(round--)
    {
        ...
        ep+=l[0];
        //XOR dengan output dari expansion pbox
        for(i=0;i<48;i++)
            exor+=char(((int(ep[i])-48)^key[round][i])+48);
        //SBOX
        for(i=0;i<48;i+=6)
        {
            row=(int(exor[i+5])-48)+(int(exor[i])-48)*2;
            col= (int(exor[i+1])-48)*8+(int(exor[i+2])-48)*4+(int(exor[i+3])-48)*2+(int(exor[i+4])-48);
            temp=s[i/6][row][col];
            soutt="";
            while(temp>0)
        ...
        r=ltem;
        cout<<"Dekripsi Round"<<16-round<<endl;
        string cip="";
        
        //Pengembalian nilai ke bentuk hex
        for(i=0;i<32;i+=4)
        {
            int te;
            te=(int(l[i])-48)*8+(int(l[i+1])-48)*4+(int(l[i+2])-48)*2+(int(l[i+3])-48);
